...
def rankings_table(df):
    """
    Display rankings table with sortOrder1 always visible.
    """
    import streamlit as st
...
# ===== Main Rankings Integration =====
def inject_sortorder1(main_rank_df, season: int, event_code: str):
    """
    Ensure sortOrder1 is present in the MAIN ranking table.
    Merges sortOrder1 from FTC rankings by team number.
    """
    import pandas as pd

    if main_rank_df is None or main_rank_df.empty:
        return main_rank_df

    try:
        rk = ftc_rankings_df(season, event_code)
    except Exception:
        return main_rank_df

    if rk is None or rk.empty or "sortOrder1" not in rk.columns:
        return main_rank_df

    team_col = None
    for c in ["team","teamNumber","team_number","number"]:
        if c in main_rank_df.columns:
            team_col = c
            break
    ...
    if "team_y" in merged.columns:
        merged = merged.drop(columns=["team_y"]).rename(columns={"team_x": team_col})
