# MAE Scouting – patch13n19d - FINAL (with RS rounding and cache improvements)
...
# Caching Improvement: Increased TTL from 900s (15 min) to 43200s (12 hours) for season-wide data
...
# Caching Improvement: Increased TTL from 180s (3 min) to 600s (10 min) for general API calls
...
# RS and Rounding Fix: TTL remains at 300s (5 min) since ranking data updates frequently
...
def ftc_rankings_df(season: int, event_code: str):
    ...
    RS value is rounded to 2 decimal places for correct display.
    ...
    for r in ranking_list:
        ...
        # FIX: RS MUST come from sortOrder1 ONLY, and is rounded to 2 decimal places
        so1 = _gi(r, "sortOrder1", "SortOrder1")
        try:
            rs = round(float(so1), 2) if so1 is not None else None # FIX: Added rounding
        except:
            rs = None
        ...
        if team is not None:
            ...
            if rs is not None: row["RS"] = rs # Use "RS" as key
...
def api_teams( eventCode: int, code: str):    return get_json_safe("http://ftc-api.firstinspires.org/v2.0/{season}/teams")


def api_matches( eventCode: int, code: str):  return get_json_safe("http://ftc-api.firstinspires.org/v2.0/{season}/matches/{eventCode}")


def api_rankings(eventCode: int, code: str): return get_json_safe("http://ftc-api.firstinspires.org/v2.0/{season}/rankings/{eventCode}")
...
def extract_RS(js): return extract_list(js, "sortOrder1")
