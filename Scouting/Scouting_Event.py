...
APP_TITLE = "MISHMASH Scouting platform (Event-Based) - Version 13n21 (V10i) (updated: 07.09.2025)"
...
def load_ftc_credentials():
    ...
    if open_picker:
        uploaded = st.file_uploader("בחר את קובץ ה-FTC credentials", type=["json", "JASON", "txt"],
...
def _safe_dataset(season: int, event_code: str, max_workers: int):
    """
    Guaranteed to return a 3-tuple of DataFrames.
    Falls back to last_dataset in session_state or empty DFs.
    """
    try:
        res = fetch_season_data(int(season), event_code, int(max_workers))
    except Exception as e:
...
@st.cache_data(ttl=900, show_spinner=False)
def _cached_dataset(season: int, event_code: str, max_workers: int):
    try:
        ev_view, base_raw, teams_master = _safe_dataset(int(season), str(event_code), int(max_workers))
    except Exception:
        if "last_dataset" in st.session_state:
            return st.session_state["last_dataset"]
        import pandas as pd
...
def _teams_seen(teams_master, base_raw):
    ...
    try:
        if getattr(base_raw, "empty", True) is False:
            ...
            for c in ["team", "teamNumber", "team_number", "number", "t1", "t2", "r1", "r2", "b1", "b2", "red1", "red2",
                      "blue1", "blue2"]:
...
def build_rankings_cached(sig, ev_view, base_raw, teams_master, event_code, season=None):
    return build_ranking_table(ev_view, base_raw, teams_master, event_code, season)


@st.cache_data(ttl=600, show_spinner=False)
def build_advancement_cached(sig, base_raw, teams_master, ev_view, event_code, season):
    try:
        if 'build_advancement' in globals():
            return globals()['build_advancement'](sig, base_raw, teams_master, ev_view, event_code, season)
        if 'compute_advancement_table' in globals():
            return globals()['compute_advancement_table'](ev_view, base_raw, teams_master, event_code, season, None)
    except Exception:
        pass
    return None
...
def _snapshot_path(kind: str, season: int, event_code: str) -> _Pth:
    safe_event = (event_code or "NA").replace(" ", "_")
    return _SNAP_DIR / f"{kind}_season{season}_{safe_event}.parquet"


def _load_snapshot(kind: str, season: int, event_code: str, max_age_sec: int = 12 * 3600):
    p = _snapshot_path(kind, season, event_code)
    if p.exists():
        try:
...
def _save_snapshot(kind: str, season: int, event_code: str, df):
    if df is None:
        return
    p = _snapshot_path(kind, season, event_code)
    try:
...
def ftc_rankings_df(season: int, event_code: str):
    ...
    for r in ranking_list:
        ...
        try:
            ...
            rs = None
        try:
            rp_total = float(rp_total) if rp_total is not None else None
        except Exception:
            rp_total = None
        try:
            ...
            matches = None
        try:
            sort1 = float(sort1) if sort1 is not None else None
        except Exception:
            sort1 = None
...
# Compute Advancement
# =======================
def build_advancement_template(ev_view, teams_master, event_code, season):
    import pandas as pd
    # We no longer filter by country
    teams_il = teams_master.copy().sort_values("team")
...
def compute_advancement_table(ev_view, base, teams_master, event_code, season, adv_csv_df):
    import pandas as pd
    # We no longer filter by country
    teams_il = teams_master.copy().sort_values("team")
    teams_il = dedupe_teams_master(teams_il, ev_view)
    ...
    # families actually present in base
    fams_base = []
    try:
...
def _load_json_credentials(path: str) -> t.Tuple[t.Optional[str], t.Optional[str]]:
    ...
    try:
        ...
        key = data.get("token") or data.get("authkey") or data.get("FTC_TOKEN") or data.get("api_key") or data.get(
            "FTC_API_KEY")
        if user and key:
...
def get_credentials() -> t.Tuple[t.Optional[str], t.Optional[str], list[str]]:
    tried = []
    try:
        gu = globals().get("FTC_USER");
        gt = globals().get("FTC_TOKEN")
...
def auth_session():
    ...
    if not (u and k):
        raise RuntimeError(
            "FTC API credentials missing (user/token). Configure ENV, secrets, or ftc_api_credentials.json")
    s = requests.Session()
    s.auth = (u, k)
...
def events_list(season: int) -> pd.DataFrame:
            ...
            for v in d.values():
                if isinstance(v, list) and (len(v) == 0 or isinstance(v[0], dict)):
                    ...
                    if any(k in cand for k in
                           ("code", "eventCode", "name", "eventName", "country", "countryCode", "startDate",
                            "dateStart")):
                        items = v;
                        break
    ...
    if df.empty:
        try:
            ...
            st.warning(
                f"Failed to list events: empty events dataframe. Check credentials/network (server ok={ok}, items={total})")
            if isinstance(d, dict):
                st.caption("Top-level keys from API:");
                st.code(", ".join(list(d.keys())[:40]))
            if keys:
                st.caption("First event keys:");
                st.code(", ".join(list(keys)[:40]))
        except Exception:
            pass
    ...
    try:
        import streamlit as st
        country_filter = (
                    st.session_state.get("country_filter") or st.session_state.get("Country") or "").strip().upper()
        if country_filter:
...
def fetch_season_data(season: int, event_code: str, max_workers: int):
    """
    Root-cause fixed loader: returns (ev_view, base_raw, teams_master).
    - Fetches data for a single specific event_code.
    - Never returns None; raises with clear message only on truly fatal issues.
    """
    _mw = int(max_workers)
    # List all events to find the one requested
    ...
    ev_f = ev_df[ev_df["event_code"].astype(str).str.upper() == str(event_code).upper()].copy()
    if ev_f.empty:
        st.warning(f"Event {event_code} not found in season {season}.")
    ...
    try:
        st.caption(
            f"📦 Loaded: matches_ok={diag_counts['matches_ok']}, teams_ok={diag_counts['teams_ok']} (errors: m={diag_counts['matches_err']}, t={diag_counts['teams_err']})")
    except Exception:
        pass
...
def build_ranking_table(ev_view: pd.DataFrame, base: pd.DataFrame, teams_master: pd.DataFrame,
                        event_code: str, season: int = None) -> pd.DataFrame:
    # We no longer filter by country, as teams_master should already be filtered for the event
    teams_master_f = teams_master.copy()
    ...
    show = ["team", "team_name", "country", "EPA", "EPA_Auto", "EPA_Teleop", "EPA_Endgame", "OPR", "sortOrder1",
            "RECORD"]
    for c in show:
...
# ---------- Diagnostics ----------
def build_diagnostics(ev_view: pd.DataFrame, base: pd.DataFrame, teams_master: pd.DataFrame, event_code: str):
    # We no longer filter by country
    teams_master_f = teams_master.copy()
    if teams_master_f.empty: return pd.DataFrame()
...
def ensure_ftc_creds_interactive(default_season=None):
    ...
    if uploaded is not None:
        try:
            ...
            if user and key:
                appdata = os.getenv("APPDATA") or ""
                target = pathlib.Path(appdata) / "mae_scout" / "ftc_api_credentials.json" if appdata else pathlib.Path(
                    "ftc_api_credentials.json")
...
if 'st' in globals():
    try:
        ensure_ftc_creds_interactive(
            default_season=(st.session_state.get('season') if hasattr(st,
                                                                      'session_state') and 'season' in st.session_state else None)
        )
...
with c1:
    season = st.number_input("Season", min_value=2006, value=DEFAULT_SEASON, step=1, key="season_input",
                             on_change=_clear_team)
with c2: event_code = st.text_input("Event Code (e.g., USTXFMS1)", "USTXFMS1", key="event_code_input",
                                 on_change=_clear_team)

MAX_WORKERS = 16
with st.spinner("Loading event data..."):
    MAXW = MAX_WORKERS
    ev_view, base_raw, teams_master = _safe_dataset(int(season), event_code, int(MAXW))
st.session_state["last_dataset"] = (ev_view, base_raw, teams_master)
...
_prefetch_key = (int(season), str(event_code))
_need_prefetch = (
...
if _need_prefetch and not base_raw.empty:
    with st.spinner("Precomputing rankings, advancements, and diagnostics"):
        with ThreadPoolExecutor(max_workers=3) as ex:
            fut_rank = ex.submit(build_ranking_table, ev_view, base_raw, teams_master, event_code, int(season))
            fut_adv = ex.submit(compute_advancement_table, ev_view, base_raw, teams_master, event_code, int(season),
                                pd.DataFrame())
            fut_diag = ex.submit(build_diagnostics, ev_view, base_raw, teams_master, event_code)
...
tab_rank, tab_adv, tab_single, tab_diag = st.tabs(
    ["🏆 Rankings", "🏅 Ranking: Advancements Points", "📅 Ranking: Single Event", "🔎 Diagnostics"])
...
with tab_rank:
    ...
    if rank_df.empty:
        st.info("No teams found for this event.")
    else:
    ...
    st.download_button("Download CSV", data=csv, file_name=f"mae_scout_rankings_{event_code}_{season}.csv",
                       mime="text/csv")
...
with tab_adv:
    ...
    st.caption(
        "חישוב נקודות עפ״י מודל ההעפלה החדש של FTC (2025‑2026): Qualification (2‑16), Alliance (21−מס׳ הברית), Playoff (40/20/10/5), Inspire (60/30/15), פרסים אחרים (12/6/3).")
    up = st.file_uploader(
        "(אופציונלי) העלה קובץ CSV עם נתוני Advancement (team,event/family,qual_rank,field_size,alliance_no,playoff_finish,awards)",
    ...
    if not isinstance(adv_summary, pd.DataFrame) or adv_summary.empty:
        with st.spinner("מחשב Advancement Points"):
            adv_summary, adv_per_team = compute_advancement_table(ev_view, base_raw, teams_master, event_code, int(season),
                                                                  adv_csv_df)
...
with tab_diag:
    with st.spinner("Computing diagnostics"):
        ...
        if _use_cached_diag:
            ...
            dtab, n_matches_opr, n_teams_opr = build_diagnostics(ev_view, base_raw, teams_master, event_code)
...
def _ensure_endgame_epa(df: pd.DataFrame, endgame_override=None, teleop_override=None) -> pd.DataFrame:
    ...
    if t_col:
        if endgame_override is not None and not (
                isinstance(endgame_override, (pd.Series, dict)) and len(endgame_override) == 0):
            df_out["EPA_Endgame"] = pd.to_numeric(df_out[t_col], errors='coerce').map(endgame_override).fillna(
                df_out.get("EPA_Endgame", np.nan))

        if teleop_override is not None and not (
                isinstance(teleop_override, (pd.Series, dict)) and len(teleop_override) == 0):
            df_out["EPA_Teleop"] = pd.to_numeric(df_out[t_col], errors='coerce').map(teleop_override).fillna(
                df_out.get("EPA_Teleop", np.nan))
...
with tab_single:
    ...
    if not selector_choices:
        ...
        if b.empty:
            ...
            if rank_df2.empty:
                sig = f"rnk|{int(season)}|{str(event_code).upper()}"
                try:
                    rank_df2 = build_rankings_cached(sig, ev_view, base_raw, teams_master, event_code, int(season))
                except Exception:
                ...
                try:
                    for (evv, idxv), rows in b.groupby(["ev", "idx"]):
                        ...
                        for col in ("t1", "t2"):
                            try:
                                red.append(int(r.get(col)));
                                blue.append(int(bl.get(col)))
                            except Exception:
                                pass
                        for t in red + blue: rec.setdefault(t, {"W": 0, "L": 0, "T": 0})
                        if abs(rs - bs) < 1e-9:
                    ...
                    with c[1]:
                        with st.expander(label):
                            ...
                            if not isinstance(st.session_state.get('per_team_detail'), dict):
                                sig_adv = f"adv|{int(season)}|{str(event_code).upper()}"
                                try:
                            ...
                            st.markdown(
                                f"• Qual: {best['qual']} | Alliance: {best['alli']} | Playoff: {best['play']} | Awards: {best['aw']}")
